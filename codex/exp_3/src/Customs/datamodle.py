...
class CalculationRequest(BaseModel):
    name: str = Field(..., description="彩票名称。如果是预设的（如'🔴双色球'），可以直接填；如果是自定义的，请起个新名字。")
...
class TaskStatus(BaseModel):
    ...
    """
    status: Literal["idle", "calculating", "done", "timeout", "error", "stopped"] = Field(
        ..., 
        description="任务的当前生命周期阶段"
    )
    elapsed_time: float = Field(
        ..., 
        description="从计算开始到现在所经过的时间（秒）"
    )
#endregion
